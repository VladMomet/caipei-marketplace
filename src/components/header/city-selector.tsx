...
const PIN_PATH = 'M7 1.5c-2.2 0-4 1.8-4 4 0 2.8 4 7 4 7s4-4.2 4-7c0-2.2-1.8-4-4-4zM7 7a1.5 1.5 0 110-3 1.5 1.5 0 010 3z'
const CHEVRON_PATH = 'M2 3.5L5 6.5L8 3.5'
...
export function CitySelector() {
  ...
  const cityLabel = mounted ? selected?.nameAcc ?? selected?.nameRu ?? '—' : '—'
    ...
    <>
      ...
      >
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
          <path d={PIN_PATH} stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" />
        </svg>
        <span className="hidden md:inline">Доставка в</span>
        <span className="max-w-[80px] truncate font-semibold sm:max-w-none">{cityLabel}</span>
        <svg
          width="10"
          height="10"
          viewBox="0 0 10 10"
        ...
        >
          <path d={CHEVRON_PATH} stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
        </svg>
      </button>
      ...
      <Sheet open={open} onOpenChange={setOpen} side="right" widthClassName="w-[380px]">
        ...
        <SheetBody className="px-3 py-3 lg:px-4 lg:py-4">
          <p className="mb-3 px-2 text-[12.5px] leading-snug text-ink-3">
            Цена в каталоге уже включает доставку. В разные города стоимость логистики разная — итог зависит от выбранного города.
          </p>
          <ul>
            {cities.map((city) => {
